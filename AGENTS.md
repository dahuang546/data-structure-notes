...
- 日记：统一使用 `2026-04-23-学习笔记.md` 单文件，按日期条目往下追加，不每天新建文件
...
- 📝 **学习笔记**：在单文件学习笔记中按日期条目追加日记，不新建每日文件；格式上日期写在条目里，不要写在文件顶部
- 📁 **目录整理**：按数据结构类型（顺序表、链表、栈、队列、串等）归类代码文件
- 🔧 **配置维护**：管理 `.gitignore`、Git 凭证、GitHub CLI 配置等
- 📋 **代码辅助**：补充注释、检查代码风格、回答数据结构相关问题
- 🔄 **自我维护**：每次推送完成后，自动回顾本轮对话内容，判断是否有新的协作策略、约定或偏好需要写入/更新到本 `AGENTS.md` 文件中，主动进行维护
